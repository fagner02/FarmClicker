...
func connect_signals():
	if !is_connected("gui_input", Callable(self, "_on_CustomFile_gui_input")):
		connect("gui_input", Callable(self, "_on_CustomFile_gui_input"))

func center_label():
	label.position.x = texture_rect.size.x * 0.5 - label.size.x * 0.5 + 4

func resize():
	var rect = label.get_rect()
	var min_x = min(label.position.x, 0)
	var max_x = min(label.position.x + label.size.x + 4, texture_rect.size.x)
	var min_y = 0
	var max_y = texture_rect.size.y + label.size.y + 6
	custom_minimum_size = Vector2(max_x - min_x + 10, max_y - min_y + 10)
	selection.custom_minimum_size = custom_minimum_size
	selection.position.x += 5
	texture_rect.position.x += 5
	label.position.x += 5
