...
onready var select_file_dialog: FileDialog = get_node("FileDialog")
...
onready var print_dict: RichTextLabel = get_node("VBoxContainer/TabContainer/Text/RichTextLabel")
onready var res_button: Button = get_node("VBoxContainer/TabContainer/Text/HBoxContainer/Button")
onready var load_res: FileDialog = get_node("FileDialog2")
...
onready var tree: Tree = get_node("VBoxContainer/TabContainer/Tree")
...
func _ready() -> void:
	...
	var add_element_button: MenuButton = get_node("VBoxContainer/Tree Manipulation Tools/AddButton")
...
func _gen_item(parent: TreeItem, type: String, value, key: String = "", has_key: bool = false) -> TreeItem:
	var valueBool : String
	
	var item := tree.create_item(parent)
	match type:
		"Boolean":
			item.set_cell_mode(VALUE_COL, TreeItem.CELL_MODE_CHECK)
			if str(value) == "True": item.set_checked(VALUE_COL, true)
			else: item.set_checked(VALUE_COL, false)
...
func _reconstruct_object(node: TreeItem) -> Dictionary:
	...
	while child:
		var valueBool: bool = child.is_checked(VALUE_COL)
		...
		match child.get_text(TYPE_COL):
			...
			"Number":
				result[key] = float(child.get_text(VALUE_COL))
			"Boolean":
...
func _reconstruct_array(node: TreeItem) -> Array:
	...
	while child:
		...
		match child.get_text(TYPE_COL):
			...
			"Number":
				result.append(float(child.get_text(VALUE_COL)))
			"Boolean":
...
func genDict(parent: TreeItem, type: String, struct):
	
	var name
	
	var child = parent.get_children()
	while child != null:
		if child.get_text(KEY_COL) == type:
			n+=1
			name = type+str(n)
		if n>0:
			var child2 = parent.get_children()
			while child2 != null:
				if child2.get_text(KEY_COL) == type+str(n):
					n+=1
					name = type+str(n)
				child2 = child2.get_next()
		child = child.get_next()

	if n == 0: name = type
	else: name == type+str(n)
	n = 0
	var temp := tree.create_item(parent)
	temp.set_cell_mode(TYPE_COL, TreeItem.CELL_MODE_STRING)
	temp.set_text(TYPE_COL, "Object")
	temp.set_editable(KEY_COL, true)
	temp.set_text(KEY_COL, name)
	
	for x in struct.keys():
		var value = struct[x]
		match typeof(value):
			TYPE_DICTIONARY:
				_gen_object(value as Dictionary, x, true, temp)
			TYPE_ARRAY:
				_gen_array(value as Array, x, true, temp)
			TYPE_STRING:	
				_gen_item(temp, "String", value, x, true)
			TYPE_BOOL:
				_gen_item(temp, "Boolean", value, x, true)
			TYPE_REAL:
				_gen_item(temp, "Number", value, x, true)
			TYPE_INT:
				_gen_item(temp, "Number", value, x, true)
			_:
				_gen_item(temp, "Null", null, x, true)
...
func _add_element(id: int, parent: TreeItem = null) -> void:
	...
	if id >5:
		print(id)
		for x in TYPES.size():
			var s: String = TYPES[x].name
			match type:
				s:
					genDict(sel, type, TYPES[x].default_val)
	else:
		match type:
			"Object":
...
func _on_Button_pressed():
	var s = load("res://Storage.tres")
	print_dict.text = str(s.main)
